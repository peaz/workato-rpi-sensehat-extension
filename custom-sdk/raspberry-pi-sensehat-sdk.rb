{
  ...
  triggers: {
    ...
    environmentalSensor: {
      title: 'New environmental sensor reading',
      description: lambda do |input, picklist_label|
        'Poll for humidity, temperature (from two sensors) and pressure reading from the SenseHAT enviromental sensor.'
      end,

      poll: lambda do |connection|
        readings = get("http://localhost/ext/#{connection['profile']}/environmentalSensor").headers('X-Workato-Connector': 'enforce')
      ...
      dedup: lambda do |readings|
        "#{readings['humidity']+readings['temperature']+readings['temperatureFromPressure']+readings['pressure']}"
      end,
      ...
      sample_output: lambda do |connection|
        {
          temperature: 41.995728,
          humidity: 50.755486,
          temperatureFromPressure: 38.972916,
          pressure: 1011.19385
        }
      end,
